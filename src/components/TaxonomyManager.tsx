  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1000px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h2 style={{ fontSize: '1.8rem', fontWeight: 800, color: '#f8fafc', marginBottom: '8px' }}>Manajemen Taksonomi Dokumen</h2>
          <p style={{ color: '#94a3b8' }}>Kelola jenis dan sub-jenis dokumen yang tersedia di sistem.</p>
        </div>
        <button
      ...
      {adding && (
        <div style={{ 
          ...
          <div style={{ flexGrow: 1 }}>
            <label style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', marginBottom: '8px' }}>Nama Jenis Dokumen</label>
            <input 
              type="text" 
              value={newName}
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-color)', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead style={{ background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid var(--border-color)' }}>
            <tr>
              <th style={{ padding: '16px 24px', fontSize: '0.85rem', color: '#94a3b8', fontWeight: 600 }}>NAMA JENIS DOKUMEN</th>
              <th style={{ padding: '16px 24px', fontSize: '0.85rem', color: '#94a3b8', fontWeight: 600 }}>STATUS</th>
              <th style={{ padding: '16px 24px', fontSize: '0.85rem', color: '#94a3b8', fontWeight: 600, textAlign: 'right' }}>AKSI</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={3} style={{ padding: '32px', textAlign: 'center', color: '#64748b' }}>Memuat taksonomi...</td></tr>
            ) : taxonomyList.length === 0 ? (
              <tr><td colSpan={3} style={{ padding: '32px', textAlign: 'center', color: '#64748b' }}>Belum ada data taksonomi</td></tr>
            ) : taxonomyList.map(tax => (
              <tr key={tax.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', opacity: tax.is_active ? 1 : 0.6 }}>
                ...
                <td style={{ padding: '16px 24px' }}>
                  <button 
                    ...
                    style={{
                      ...
                      color: tax.is_active ? '#22c55e' : '#94a3b8',
